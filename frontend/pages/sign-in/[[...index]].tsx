...
export function SignInPage({ locale: serverLocale }: { locale?: string }) {
  ...
  useEffect(() => {
    setMounted(true);
    logger.debug("Sign-in page mounted, router locale:", router.locale);
  }, [router.locale]);
...
export async function getServerSideProps({ locale }: { locale: string }) {
  logger.debug("Server side locale for sign-in:", locale);
  return {
    props: {
      ...(await serverSideTranslations(locale || 'zh', ['common', 'sign-in'])),
      locale
    }
  };
} 