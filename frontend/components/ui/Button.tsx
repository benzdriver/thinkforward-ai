...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ as: Component = 'button', href, ...props }, ref) => {
    ...
    const variantClasses = {
      ...
      'outline-white': 'border-white text-white hover:bg-white/10 bg-transparent',
    };
    ...
    // 禁用状态样式
    const disabledClass = (props.disabled || props.isLoading) ? 'opacity-50 cursor-not-allowed' : '';
    ...
    return (
      <Component
        ref={ref}
        href={href}
        className={`inline-flex items-center justify-center border border-transparent rounded-md font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors ${variantClasses[props.variant || 'primary']} ${sizeClasses[props.size || 'md']} ${disabledClass} ${widthClass} ${props.className || ''}`}
      ...
      >
        ...
        {props.icon && props.iconPosition === 'left' && !props.isLoading && (
